    ...
    <div className="max-w-5xl mx-auto space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          ...
          >
            ...
            <div className="text-2xl font-black tabnum" style={{ color: "var(--color-navy)" }}>{s.val}</div>
      ...
      <div>
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-lg font-black text-[var(--color-navy)] tracking-tight" style={{ fontFamily: "var(--font-display)" }}>Patient Roster</h2>
          <Link
      ...
      >
        ...
        <div className="space-y-3">
          {MOCK_PATIENTS.map((p, i) => {
            const [date, time] = p.nextAppointment.split(" ");
            const color = AVATAR_COLORS[i % AVATAR_COLORS.length];
            const initials = p.name.split(" ").map((w) => w[0]).join("").slice(0, 2);
            const status = apptStatus[p.id] ?? null;
            return (
              <div key={p.id} className="flex items-center gap-3 p-2 rounded-xl transition-all"
                style={{ background: status === "confirmed" ? "rgba(16,185,129,0.05)" : status === "cancelled" ? "rgba(239,68,68,0.05)" : "transparent" }}>
                <div
